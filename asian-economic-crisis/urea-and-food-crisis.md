---
type: analysis
title: Urea and Food Crisis
description: The Hormuz energy crisis has cascaded into a food crisis through the gas → urea → fertiliser → harvest chain.
bundle: political-economy
topic: asian-economic-crisis
tags:
- commodity-shock
- energy-markets
- china
- geopolitics
- food-security
sources:
- id: multiple-press-reports-fao-reuters-regional-media-compiled-april-2026
  resource: Multiple press reports (FAO, Reuters, regional media) — compiled April 2026
generated:
  by: claude-code/atlas-consolidate
  at: '2026-05-09T07:13:14Z'
status: stable
related:
- political-economy/hormuz-strait-crisis/hormuz-blockade-overview.md
- political-economy/energy-markets-2026/lng-crisis-and-gas-markets.md
- political-economy/asian-economic-crisis/china-strategic-positioning.md
- political-economy/asian-economic-crisis/asian-energy-crisis-overview.md
- political-economy/asian-economic-crisis/country-impacts.md
---

# Urea and Food Crisis

**Source:** Multiple press reports (FAO, Reuters, regional media) — compiled April 2026
**Author:** (source: needs-verification)

---

## Summary

The Hormuz energy crisis has cascaded into a food crisis through the gas → urea → fertiliser → harvest chain. Urea prices rose 83% in the first quarter of 2026, with multiple Asian countries facing fertiliser reserves measured in weeks rather than months, threatening entire harvest cycles if supplies don't arrive in time for planting.

The energy crisis is metamorphosing into a food crisis through a critical dependency: natural gas → urea → nitrogen fertilisers → food production.

## The Urea Chain

- Natural gas is 60–90% of urea production costs (source: urea-and-food-crisis.md)
- The Persian Gulf supplies 35% of global urea exports (source: urea-and-food-crisis.md)
- Urea is the basis of nitrogen fertilisers essential for rice, corn, and wheat in Asia (source: urea-and-food-crisis.md)
- By 2026-03-31, urea prices had risen **83% since 1 January 2026** (source: urea-and-food-crisis.md)

## Country Exposure

- **Thailand**: relies on the Middle East for 55% of nitrogen-based raw material imports; fertiliser reserves last until August
- **Indonesia**: Pupuk (state-owned) produces urea domestically, but blending companies have only 45 days of reserves; paradox — producers tempted to export for cash rather than supply domestically
- **India**: imports 40% of urea needs from the Middle East; had hoped for Chinese supplies
- **Philippines**: national state of emergency declared; LPG prices surged from 1,000 to 1,400 pesos, crushing small food vendors ("carinderias" employing ~1M workers)

## China's Role

- Halted all fertiliser exports (~40M tons to stay within Chinese borders)
- Accumulated 3–4 months of oil reserves
- Protects domestic food security while tightening the noose on the rest of Asia
- International urea prices up 40% partly due to Chinese export ban

## Key Takeaways

- Gas → urea → fertiliser → food: the Hormuz blockade is a systemic trap that hits agriculture
- If fertilisers don't arrive in time for planting, entire harvests are lost — not just a price problem
- China's export ban on fertilisers is a strategic move that worsens the crisis for the rest of Asia
- Multiple countries have reserves measured in weeks, not months

## Related

- [[hormuz-blockade-overview]] — root cause of the gas → urea → food cascade
- [[lng-crisis-and-gas-markets]]
- [[china-strategic-positioning]]
- [[asian-energy-crisis-overview]]
- [[country-impacts]]

## Tags

#political-economy #asian-economic-crisis #food-security #urea #fertiliser #china
