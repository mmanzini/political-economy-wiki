---
type: analysis
title: US Macro Data — April 2026
description: US macro data for March/April 2026 shows the largest monthly CPI increase since 2022 (+3.3%, energy-driven), collapsing rate-cut expectations (from 56% to 30% probability by year-end), and a Q4 GDP slowdown (0.5% vs Q3's 4.4%) exacerbated by the longest government shutdown in US history.
bundle: political-economy
topic: us-markets-and-economy
tags:
- financial-markets
- us-politics
- energy-markets
- commodity-shock
sources:
- id: multiple-sources-bls-fed-university-of-michigan-imf-compiled-april-2026
  resource: Multiple sources (BLS, Fed, University of Michigan, IMF) — compiled April 2026
generated:
  by: claude-code/atlas-consolidate
  at: '2026-05-09T07:18:11Z'
status: stable
related:
- political-economy/us-markets-and-economy/us-equities-april-2026.md
- political-economy/european-economy/european-stagflation-risk.md
- political-economy/energy-markets-2026/energy-supply-disruption.md
---

# US Macro Data — April 2026

**Source:** Multiple sources (BLS, Fed, University of Michigan, IMF) — compiled April 2026
**Author:** (source: needs-verification)

---

## Summary

US macro data for March/April 2026 shows the largest monthly CPI increase since 2022 (+3.3%, energy-driven), collapsing rate-cut expectations (from 56% to 30% probability by year-end), and a Q4 GDP slowdown (0.5% vs Q3's 4.4%) exacerbated by the longest government shutdown in US history.

## Inflation

- March CPI expected at **3.3%** (up from 2.4%) — largest monthly increase since 2022 (source: us-macro-data-april-2026.md)
- Energy projected to contribute +10.6% (source: us-macro-data-april-2026.md)
- Core inflation expected at 2.7% (up from 2.5%) (source: us-macro-data-april-2026.md)
- PCE price index: +0.4% in February (source: us-macro-data-april-2026.md)

## GDP

- Q4 GDP steady at 0.5% (Q3 was 4.4%)
- Consumption slowing: 3.5% → 1.9%
- Private investment rose 2.3%
- Longest government shutdown in history occurred in Q4

## Labour Market

- Initial jobless claims at two-year lows

## Fed and Rates

- 10-year Treasury yield: 4.29% (staying below 4.3%)
- 30-year bond auction: yield up 0.5% vs. last issuance
- Rate cut probability by year-end: dropped from 56% to **30%** in two days
- Dollar index heading for weekly decline of ~1.5%
- Euro/dollar below 1.17

## Fed Leadership

- Senate hearing on Kevin Warsh postponed (paperwork issues)
- Senator Tom Tillis refuses to vote until DOJ case against Jerome Powell is resolved

## Consumer Confidence

- University of Michigan preliminary: expected 52.9 (down from 53)

## IMF (Spring Meetings Preview)

- Kristalina Georgieva: global growth forecasts to be cut (was projected at 3.3%)
- Warned governments: only use targeted/temporary support if fiscal space exists
- Avoid deficit-increasing measures that fuel inflation
- Central banks advised to wait before intervening

## Key Takeaways

- March CPI expected to show largest monthly jump since 2022 — energy-driven
- Rate cut expectations collapsing (56% → 30%) as inflation rises
- US economy resilient compared to Asia (net exporter advantage) but consumption weakening
- IMF signalling global growth downgrades at Spring Meetings

## Related

- [[us-equities-april-2026]]
- [[european-stagflation-risk]]
- [[energy-supply-disruption]]

## Tags

#political-economy #us-markets #economy #inflation #cpi #fed #2026
